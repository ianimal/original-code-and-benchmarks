...
runTimeLog=${workDir}${nameRegion}.HC.log.txt
...
function GATKpipeline () {
	...
	function HaplotypeCaller () {
		...
		-o ${workDir}ALL.${nameRegion}.HC.raw.vcf \
		#--bamOutput ${workDir}ALL.${region}.HaplotypeCaller.bam \
		#-bamWriterType CALLED_HAPLOTYPES \
		-l INFO \
		-allowPotentiallyMisencodedQuals \
		-log ${workDir}ALL.${region}.HaplotypeCaller.log
		RIGHT_NOW=$(date +"%s")
		echo "Ending: $FUNCNAME" $RIGHT_NOW >> ${runTimeLog}
	}
	HaplotypeCaller
	bzip2 ${workDir}ALL.${nameRegion}.HC.raw.vcf
