...
runTimeLog=${workDir}${nameRegion}.samtools.log.txt
...
function GATKpipeline () {
	...
	function mpileup () {
    	RIGHT_NOW=$(date +"%s")
		echo "Beginning: $FUNCNAME" $RIGHT_NOW >> ${runTimeLog}
   		${SAMTOOLS}samtools mpileup -f ${reference} -r ${region} -A -C50 -E -D -S -u \
   		...
   		${BCFTOOLS}bcftools view -cvgb - > ${workDir}ALL.${nameRegion}.raw.bcf 2> ${workDir}ALL.${nameRegion}.log
   		${BCFTOOLS}bcftools view ${workDir}ALL.${nameRegion}.raw.bcf | ${vcfutils} varFilter -D50000 - > ${workDir}ALL.${nameRegion}.flt.vcf
   		RIGHT_NOW=$(date +"%s")
		echo "Ending: $FUNCNAME" $RIGHT_NOW >> ${runTimeLog}
	}
	mpileup
	bzip2 ${workDir}ALL.${nameRegion}.flt.vcf
